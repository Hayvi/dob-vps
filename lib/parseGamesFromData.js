function parseGamesFromData(rawData, sportName = "Unknown") {
    ...
    const pushGame = (game, regionName, competitionName, competitionData = {}, regionData = {}) => {
        const markets = {};
        if (game.market) {
        ...
        allGames.push({
            ...
            sport: sportName,
            region: regionName,
            region_order: regionData.order,
            competition: competitionName,
            competition_favorite: competitionData.favorite,
            competition_favorite_order: competitionData.favorite_order,
            competition_order: competitionData.order,
            teams_reversed: competitionData.teams_reversed,
            market: markets
        });
    };
        ...
        for (const regionId in data.region) {
            ...
            for (const competition of competitions) {
                ...
                for (const game of games) {
                    pushGame(game, region?.name || regionId, competition?.name, competition, region);
                }
            }
        }
    }
            ...
            if (sport?.region) {
                ...
                for (const region of regions) {
                    ...
                    for (const competition of competitions) {
                        ...
                        for (const game of games) {
                            pushGame(game, region?.name, competition?.name, competition, region);
                        }
                    }
                }
                ...
                for (const competition of competitions) {
                    const games = resolveCollection(competition?.game, sport.game || data.game);
                    for (const game of games) {
                        pushGame(game, sport.name, competition?.name, competition, {});
                    }
                }
            }
        }
