...
class ScraperProxy {
  ...
  async subscribeToGameMarkets(gameId, onUpdate) {
    const request = { source: 'betting', what: { market: ['id','name','type','order','col_count','display_key','is_blocked'], event: ['id','name','price','order','type','base','is_blocked'] }, where: { game: { id: parseInt(gameId) } }, subscribe: true };
    return this.subscribe(request, onUpdate);
  }
